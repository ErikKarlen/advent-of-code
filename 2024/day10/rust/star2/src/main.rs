...
fn main() {
    ...
    {
        let mut trails_to_check = vec![vec![trailhead]];
        let mut trails: HashSet<Vec<(i32, i32)>> = HashSet::new();
        while !trails_to_check.is_empty()
        {
            let curr_trail = trails_to_check.pop().unwrap();
            let curr_pos = curr_trail.last().unwrap();
            let curr_height = topographic_map[curr_pos.1 as usize][curr_pos.0 as usize];
            if curr_height == 9 {
                trails.insert(curr_trail);
                continue;
            }
            //dbg!(curr_pos);
            for dir in &dirs
            {
                ...
                if 0 <= next_pos.0 && next_pos.0 < input_width && 0 <= next_pos.1 && next_pos.1 < input_height
                {
                    ...
                    {
                        let mut next_trail = curr_trail.clone();
                        next_trail.push(next_pos);
                        trails_to_check.push(next_trail);
                    }
                }
            }
        }
    ...
    println!("The sum of the ratings of all trailheads on the topographic map is {total_rating}.");
}
